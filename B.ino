...
void loop() {
  ...
  if (myFile) {
    ...
    // If the sensor loaded ok, save following values for humidity, pressure and temperature:
    myFile.print(mySensor.readFloatHumidity(), 2);
    myFile.print(", ");

    myFile.print(mySensor.readTempF(), 2);
    myFile.print(", ");

    myFile.print(mySensor.readFloatPressure(), 2);
    myFile.println();
  ...
  // Delay for five sec during daytime
  
  if (now.hour() < 6) {  // If time less than 6
    delay(600000);
    logId += 1;

  } else { // If time bigger than 6
    delay(300000);
    logId += 1;
  }
